...
In order to construct the file by hand, you'll first want to grab a project version or branch to ensure that your docs sync to the right place in your developer hub. That version or branch will be passed via the `--branch` flag. See below for a full example:
  ...
  sync:
    ...
    steps:
      ...
      - name: GitHub Action
        ...
        with:
          rdme: docs ./documentation --key=${{ secrets.README_API_KEY }} --branch=2.0
```
