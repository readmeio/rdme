...
describe('rdme openapi', () => {
  ...
  it('should POST to the API if no ID provided', () => {
    const mock = nock(config.host)
      .get('/api/v1/api-specification')
      .basicAuth({ user: key })
      .reply(200, [])
  ...
  it('should PUT to the API if an ID is provided', () => {
    ...
    return openapi
      .run({ spec: require.resolve('@readme/oas-examples/2.0/json/petstore.json'), key, id, version })
      .then(() => {
        mock.done();
      });
  });
